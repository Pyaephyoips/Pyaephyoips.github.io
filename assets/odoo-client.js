...
async function directBuildInventoryReport(cfg, uid, params) {
  ...
  const byProduct = await directReadGroup(cfg, uid, 'stock.quant', internalDomain, ['quantity'], ['product_id']);

  let totalValue = 0;
  const productIds = byProduct.filter(r => r.product_id).map(r => r.product_id[0]);
  const products = productIds.length
    ? await directExecuteKw(cfg, uid, 'product.product', 'read', [productIds, ['standard_price', 'categ_id']])
    : [];
...
async function directBuildInventoryMovementReport(cfg, uid, params) {
  const companyId = directGetCompanyId(params);
  const { dateFrom, dateTo } = directResolveRange(params);
  const doneWindow = [['state', '=', 'done'], ['date', '>=', dateFrom], ['date', '<=', dateTo]];

  const grnDomain = directWithCompany([...doneWindow, ['picking_type_id.code', '=', 'incoming'], ['origin_returned_move_id', '=', false]], companyId);
  const ginDomain = directWithCompany([...doneWindow, ['picking_type_id.code', '=', 'outgoing'], ['origin_returned_move_id', '=', false]], companyId);
  const returnedDomain = directWithCompany([...doneWindow, ['origin_returned_move_id', '!=', false]], companyId);
  const adjustmentDomain = directWithCompany([
    ...doneWindow, ['picking_id', '=', false],
    '|', ['location_dest_id.usage', '=', 'inventory'], ['location_id.usage', '=', 'inventory'],
  ], companyId);
  const scrapDomain = directWithCompany([['state', '=', 'done'], ['date_done', '>=', dateFrom], ['date_done', '<=', dateTo]], companyId);
  ...
  const [
    ...
    directReadGroup(cfg, uid, 'stock.move', grnDomain, ['product_qty'], []),
    directReadGroup(cfg, uid, 'stock.move', grnDomain, ['product_qty'], ['date:month']),
    directReadGroup(cfg, uid, 'stock.move', grnDomain, ['product_qty'], ['product_id']),
    directReadGroup(cfg, uid, 'stock.move', ginDomain, ['product_qty'], []),
    directReadGroup(cfg, uid, 'stock.move', ginDomain, ['product_qty'], ['date:month']),
    directReadGroup(cfg, uid, 'stock.move', ginDomain, ['product_qty'], ['product_id']),
    directReadGroup(cfg, uid, 'stock.move', returnedDomain, ['product_qty'], []),
    directReadGroup(cfg, uid, 'stock.move', returnedDomain, ['product_qty'], ['date:month']),
    directReadGroup(cfg, uid, 'stock.move', adjustmentDomain, ['product_qty'], []),
    directReadGroup(cfg, uid, 'stock.move', adjustmentDomain, ['product_qty'], ['date:month']),
    directReadGroup(cfg, uid, 'stock.scrap', scrapDomain, ['scrap_qty'], []),
    directReadGroup(cfg, uid, 'stock.scrap', scrapDomain, ['scrap_qty'], ['date_done:month']),
    directReadGroup(cfg, uid, 'stock.scrap', scrapDomain, ['scrap_qty'], ['product_id']),
  ]);
  ...
  const productLabel = (rows, qtyField) => directTopN(rows, qtyField).map(r => ({
...
async function fetchOdooReportDirect(path, params, cfg) {
  ...
  switch (path) {
    ...
    case '/api/inventory-movement': return directBuildInventoryMovementReport(cfg, uid, params);
